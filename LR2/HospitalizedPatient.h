...
public:
    HospitalizedPatient(const string& name, const string& surname, const string& patronymic, int year_birth,
        ...
        : Patient(name, surname, patronymic, year_birth, gender, address, phone, insurance), ward(ward), 
    ...
    HospitalizedPatient& operator=(const HospitalizedPatient& other) {
        if (this != &other) {
            Patient::operator=(other);
            ward = other.ward;
            daysInHospital = other.daysInHospital;
        }
        return *this;
    }
